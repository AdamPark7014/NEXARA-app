...
6b. **Cámara en vivo (petición de Adam, 14-09 10:30):** la vista previa no bastaba porque la foto salía «como flash» (`capturePhoto` abría la cámara y disparaba sola a los 100 ms). Ahora `ActivityEvidenceFlow` abre la cámara en vivo (`liveKind`, `<video>` con frontal en espejo, «📸 Tomar foto», «🔄 Usar frontal/trasera», «Cancelar»); `grabFrame` toma el cuadro (hasta 1280 px, JPEG 0.6) y pasa a la vista previa con ubicación; «Tomar otra» vuelve a la cámara en vivo. El GPS se pide al abrir la cámara (`liveGeoRef`). Las dos ventanas se montan con `createPortal(…, document.body)`: dentro de la tarjeta la capa quedaba atrapada y se encimaba con «Sin archivos adjuntos».
...
- Cámara en vivo (10:36): `tsc` web sin errores nuevos; sin restos de `capturePhoto`/`captureWithLocation`; `docker compose build web` + `up -d`; el bundle trae «Tomar foto», «Usar frontal», «Ver en mapa» y «Al tomar la foto se guarda»; `/erp/actividades/1/evidencias` 200.
