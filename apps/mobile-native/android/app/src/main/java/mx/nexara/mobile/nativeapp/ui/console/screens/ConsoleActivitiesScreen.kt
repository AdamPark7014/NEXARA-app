...
class ConsoleActivitiesViewModel(app: Application) : AndroidViewModel(app) {
    ...
    fun refresh(scope: String? = null) {
        ...
        viewModelScope.launch {
            try {
                val list = withContext(Dispatchers.IO) { repo.activitiesFetch(scope = scope) }
                _state.update { it.copy(isLoading = false, activities = list, error = null) }
            } catch (e: Exception) {
                _state.update {
                    it.copy(
                        isLoading = false,
                        error = e.message?.takeIf { msg -> msg.isNotBlank() } ?: "No se pudieron cargar actividades",
                    )
                }
            }
        }
...
fun ConsoleActivitiesScreen(
    ...
    if (state.activities.isEmpty() && state.isLoading && state.error == null) {
        vm.refresh(scope = scope)
    }

    val normalizedQ = state.query.trim().lowercase()
    val filtered = if (normalizedQ.isBlank()) state.activities else state.activities.filter { a ->
        val hay = buildString {
            append(a.titulo ?: "")
            append(" ")
            append(a.estatus)
            append(" ")
            append(a.responsable?.nombre ?: "")
        }.lowercase()
        hay.contains(normalizedQ)
    }
    ...
    Column(
        ...
        if (state.isLoading) {
            Text("Cargando actividades...", color = MaterialTheme.colorScheme.onSurfaceVariant)
            return@Column
        }
        ...
        if (!state.error.isNullOrBlank()) {
            ...
            Button(onClick = { vm.refresh(scope = scope) }) { Text("Reintentar") }
            return@Column
        }
            ...
            items(filtered.take(200)) { a ->
                Card(
                    ...
                    Column(modifier = Modifier.padding(12.dp)) {
                        ...
                        Text(
                            text = "Estatus: ${a.estatus}",
                            style = MaterialTheme.typography.bodySmall,
                            color = MaterialTheme.colorScheme.onSurfaceVariant,
                        )
                        val owner = a.responsable?.nombre
