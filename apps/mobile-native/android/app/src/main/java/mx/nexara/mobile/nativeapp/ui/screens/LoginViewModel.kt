package mx.nexara.mobile.nativeapp.ui.screens

import android.app.Application
import androidx.fragment.app.FragmentActivity
import androidx.lifecycle.AndroidViewModel
import androidx.lifecycle.viewModelScope
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.flow.MutableStateFlow
import kotlinx.coroutines.flow.StateFlow
import kotlinx.coroutines.flow.update
import kotlinx.coroutines.launch
import kotlinx.coroutines.withContext
import mx.nexara.mobile.nativeapp.data.AuthErrorMapper
import mx.nexara.mobile.nativeapp.data.AuthRepository
import mx.nexara.mobile.nativeapp.data.LoginPreferencesStore
import mx.nexara.mobile.nativeapp.data.QuickProfile
import mx.nexara.mobile.nativeapp.push.PushRegistration
import mx.nexara.mobile.nativeapp.security.AppLock

data class LoginUiState(
    val email: String = "",
    val password: String = "",
    val quickProfiles: List<QuickProfile> = emptyList(),
    val isLoading: Boolean = false,
    val isBiometricLoading: Boolean = false,
    val showBiometricOption: Boolean = false,
    val error: String? = null,
    val infoMessage: String? = null,
)

class LoginViewModel(app: Application) : AndroidViewModel(app) {
    private val repo = AuthRepository(app.applicationContext)
    private val loginPrefs = LoginPreferencesStore(app.applicationContext)

    private val _state = MutableStateFlow(
        LoginUiState(
            quickProfiles = repo.quickProfiles(),
            showBiometricOption = AppLock.canAuthenticate(app.applicationContext) &&
                repo.quickProfiles().isNotEmpty(),
        ),
    )
    val state: StateFlow<LoginUiState> = _state

    init {
        viewModelScope.launch {
            loginPrefs.lastEmail.collect { savedEmail ->
                if (savedEmail.isNotBlank()) {
                    _state.update { current ->
                        if (current.email.isBlank()) {
                            current.copy(email = savedEmail, error = null)
                        } else {
                            current
                        }
                    }
                }
            }
        }
    }

    fun setEmail(value: String) = _state.update { it.copy(email = value, error = null, infoMessage = null) }
    fun setPassword(value: String) = _state.update { it.copy(password = value, error = null, infoMessage = null) }

    fun selectQuickProfile(profile: QuickProfile) {
        _state.update {
            it.copy(
                email = profile.email,
                error = null,
                infoMessage = null,
            )
        }
    }

    fun submit(onLoggedIn: () -> Unit) {
        val snapshot = _state.value
        if (snapshot.isLoading || snapshot.isBiometricLoading) return
        if (snapshot.email.isBlank() || snapshot.password.isBlank()) return

        _state.update { it.copy(isLoading = true, error = null, infoMessage = null) }
        viewModelScope.launch {
            try {
                val trimmedEmail = snapshot.email.trim()
                withContext(Dispatchers.IO) {
                    repo.login(trimmedEmail, snapshot.password)
                }
                loginPrefs.saveLastEmail(trimmedEmail)
                PushRegistration.registerCurrentDeviceAsync(getApplication<Application>().applicationContext)
                _state.update {
                    it.copy(
                        isLoading = false,
                        error = null,
                        infoMessage = null,
                        quickProfiles = repo.quickProfiles(),
                        showBiometricOption = AppLock.canAuthenticate(getApplication()) &&
                            repo.quickProfiles().isNotEmpty(),
                    )
                }
                onLoggedIn()
            } catch (e: Exception) {
                _state.update {
                    it.copy(
                        isLoading = false,
                        error = AuthErrorMapper.loginMessage(e),
                    )
                }
            }
        }
    }

    fun unlockWithBiometric(activity: FragmentActivity) {
        val snapshot = _state.value
        if (!snapshot.showBiometricOption || snapshot.isLoading || snapshot.isBiometricLoading) return
        val profile = snapshot.quickProfiles.firstOrNull() ?: return

        _state.update { it.copy(isBiometricLoading = true, error = null, infoMessage = null) }
        viewModelScope.launch {
            val unlocked = AppLock.authenticate(
                activity = activity,
                title = "Acceso rápido a NEXARA",
                subtitle = "Confirma tu identidad para continuar",
            )
            _state.update {
                if (unlocked) {
                    it.copy(
                        isBiometricLoading = false,
                        email = profile.email,
                        infoMessage = "Perfil seleccionado. Ingresa tu contraseña para continuar.",
                        error = null,
                    )
                } else {
                    it.copy(
                        isBiometricLoading = false,
                        error = "No se pudo verificar tu identidad. Intenta de nuevo.",
                    )
                }
            }
        }
    }
}
