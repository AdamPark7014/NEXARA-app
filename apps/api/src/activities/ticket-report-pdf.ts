...
import { cacheKeyFor, getStaticMapImage, isUsableCoordinate } from '../common/maps/static-map-cache.js';
...
/**
 * Mapa de llegada del PDF.
 *
 * Cada reporte pedía la imagen a Google otra vez —y una vez por cada evidencia
 * PDF del mismo ticket, con la misma coordenada—, así que un solo ticket con
 * tres hojas de servicio compraba tres imágenes idénticas. Ahora pasa por la
 * caché en disco (`common/maps/static-map-cache.ts`) y además se memoriza
 * dentro del propio reporte: como mucho una compra por coordenada nueva.
 */
const fetchArrivalMapImage = async (
  memo: Map<string, Buffer | null>,
  lat?: number | null,
  lng?: number | null,
): Promise<Buffer | null> => {
  if (!isUsableCoordinate(lat ?? null, lng ?? null)) return null;
  const request = { lat: lat as number, lng: lng as number, zoom: 16, width: 600, height: 600 };
  const memoKey = cacheKeyFor(request);
  if (memo.has(memoKey)) return memo.get(memoKey) ?? null;
  const image = await getStaticMapImage(request);
  const buffer = image?.buffer ?? null;
  memo.set(memoKey, buffer);
  return buffer;
};
...
export const generateTicketReportPdf = async (payload: TicketReportPayload): Promise<Buffer> => {
  return new Promise((resolve, reject) => {
    ...
    /** Una coordenada, una imagen: el mismo punto no se pide dos veces por PDF. */
    const arrivalMapMemo = new Map<string, Buffer | null>();
    ...
    const drawEvidences = async () => {
      for (const evidence of payload.evidences) {
        ...
        if (mapForPdf) {
          const mapBuffer = await fetchArrivalMapImage(
            arrivalMapMemo,
            arrivalCoordinates.latitud,
            arrivalCoordinates.longitud,
          );
          if (mapBuffer) {
