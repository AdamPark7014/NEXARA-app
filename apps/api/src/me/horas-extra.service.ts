...
export class HorasExtraService {
  ...
  async listar(
    ...
    const filas = await this.prisma.overtimeApproval.findMany({
      where: {
        ...
        // Sin empresa activa no se devuelve nada: el alcance del organigrama no
        // distingue empresas, y este sistema ya tuvo un incidente de datos cruzados.
  ...
  async guardarHorario(
    ...
    if (vacio) {
      // Solo la fila de esta empresa: borrar por `userId` a secas dejaría sin horario a
      // la misma persona en otra empresa.
      await this.prisma.workSchedule.deleteMany({ where: { userId, ...companyWhere(tenantId) } });
