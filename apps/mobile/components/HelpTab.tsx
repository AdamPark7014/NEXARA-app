...
const helpContent: Record<string, Record<string, string>> = {
  ...
  settings: {
    superadmin: `Este módulo almacena parámetros del sistema como pares clave → valor en la base de datos (tabla system_settings). Cada entrada tiene: clave (identificador único, sin espacios, ej: company_name), valor (el dato que usa el sistema), categoría (agrupa las claves en pestañas) y etiqueta opcional (nombre legible que se muestra en la UI en lugar de la clave técnica).\n\nCómo operar:\n• Editar: selecciona la categoría en el panel izquierdo, modifica el valor en el campo de texto y presiona "Guardar". El cambio es inmediato en todo el sistema.\n• Crear nueva clave: al fondo del panel rellena clave (usa guiones_bajos), valor, etiqueta opcional y presiona "Agregar". La clave debe ser única.\n• Eliminar: presiona el ícono de basura. No hay vuelta atrás.\n• Si una categoría no aparece en el menú izquierdo es porque aún no tiene claves creadas.\n\nCategorías predefinidas: General (comportamiento base), Empresa (nombre, RFC, dirección, teléfono), Fiscal (datos fiscales, régimen), Notificaciones (umbrales de alertas, emails receptores), Seguridad (intentos de login, expiración de sesión). Puedes crear categorías propias simplemente escribiéndolas al agregar una clave nueva.`,
