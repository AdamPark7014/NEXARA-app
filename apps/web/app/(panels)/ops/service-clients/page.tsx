  ...
  const columns: Column<ServiceClient>[] = [
    ...
    { key: "id", label: "", render: c => (
      <div style={{ display: "flex", gap: 4 }}>
        <button onClick={() => openEdit(c)} title="Editar" style={{ background: "none", border: "none", cursor: "pointer", fontSize: 15, color: "var(--text-tertiary)", padding: "4px 6px" }}>✎</button>
        <button onClick={() => remove(c.id)} title="Dar de baja" style={{ background: "none", border: "none", cursor: "pointer", fontSize: 15, color: "var(--text-tertiary)", padding: "4px 6px" }}>✕</button>
      </div>
    ), width: 60 },
  ];
  ...
  return (
    <>
      <PageHeader
        ...
        actions={<Button variant="primary" iconLeft="+" onClick={openNew}>Agregar cliente</Button>}
      />
