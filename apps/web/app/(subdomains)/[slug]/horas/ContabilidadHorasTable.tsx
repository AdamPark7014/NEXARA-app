...
const ContabilidadHorasTable = () => {
  ...
  const summary = useMemo(() => {
    ...
    const totalMinutesAll = visibleUsers.reduce((sum, item) => sum + (item.totalMinutes || 0), 0);
    const avgMinutesPerUser = totalUsers ? Math.round(totalMinutesAll / totalUsers) : 0;
    return { totalUsers, totalMinutesAll, avgMinutesPerUser };
  }, [visibleUsers]);
  ...
  return (
    <section className={styles.card}>
      <header className={styles.hero}>
        <div>
          <p className={styles.kicker}>Analitica financiera</p>
          <h2 className={styles.title}>Estadísticas de asistencia</h2>
          <p className={styles.subtitle}>
            Observa jornadas, productividad y actividad diaria con detalle profesional.
          </p>
        </div>
...
export default ContabilidadHorasTable;
