  ...
  const downloadReport = async (id: number) => {
    try {
      ...
      if (res.ok) {
        ...
        a.href = url;
        a.download = `reporte-cliente-${id}.pdf`;
        a.click();
      ...
      <div style={{ display: 'grid', gap: 24 }}>
        ...
        <div className="card" style={{ padding: 16, overflowX: 'auto' }}>
          ...
          {loading ? (
            ...
            <RoleGuard permissions={[PERMISSIONS.CONSOLE_ADMIN]}>
              <div style={{ display: 'grid', gap: 24 }}>
                <HelpTab module="service-clients" user={user} />
                {/* KPI Cards */}
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16 }}>
                  <div className="card" style={{ padding: 16, textAlign: 'center' }}>
                    <div style={{ fontSize: 28, fontWeight: 700, color: 'var(--primary)' }}>{clients.length}</div>
                      ...
                      <tbody>
                        {clients.map((c) => (
                          <tr key={c.id} style={{ borderBottom: '1px solid var(--border)' }}>
                            <td style={{ padding: '8px 6px', fontFamily: 'monospace' }}>{c.id}</td>
                            <td style={{ padding: '8px 6px' }}>{c.nombre || c.name}</td>
                            ...
                            <td style={{ padding: '8px 6px' }}>
                              <span style={{ color: c.activo !== false ? 'var(--success, #22c55e)' : 'var(--danger, #ef4444)' }}>
                                {c.activo !== false ? 'Sí' : 'No'}
                              </span>
                            </td>
                            <td style={{ padding: '8px 6px' }}>
                              <button onClick={() => downloadReport(c.id)} style={{ padding: '4px 10px', borderRadius: 6, background: 'var(--primary)', color: '#fff', border: 'none', cursor: 'pointer', fontSize: 13 }}>
                                Descargar reporte
                              </button>
                            </td>
                          </tr>
