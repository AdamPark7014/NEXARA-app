...
export const ORIGEN_ETIQUETA: Record<string, string> = {
  ANDROID: "App Android",
  IOS: "App iPhone",
  WEB: "Navegador",
};
...
export async function descargarPreNominaExcel(token: string, rango: { desde: string; hasta: string }) {
  ...
  const res = await fetch(buildApiUrl(`employee-payments/pre-nomina/export.xlsx?${q}`), {
    credentials: "include",
    // Sin `X-Company-Id` el servidor resuelve la empresa por membresía: quien tiene dos
    // se bajaría la nómina de la otra sin enterarse. Esto es dinero, no un listado.
